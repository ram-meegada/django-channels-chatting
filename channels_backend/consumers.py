...
class AgentChatbotUserChatting(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if self.talking_with_agent == True:
            # try:
            save_customer_conversation = await database_sync_to_async(ChatStorageWithSessionIdModel.objects.create)(session_id=self.session_foreign_key, user_input=f'{self.username_in_chatting[1]}: {text_data}')
